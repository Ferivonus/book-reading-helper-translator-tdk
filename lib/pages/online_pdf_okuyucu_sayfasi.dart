...
import 'package:syncfusion_flutter_pdf/pdf.dart'; // Metin ayıklama için
import 'package:http/http.dart' as http; // Online PDF verisini çekmek için

class OnlinePdfOkuyucuSayfasi extends StatefulWidget {
  final String pdfUrl;
  const OnlinePdfOkuyucuSayfasi({super.key, required this.pdfUrl});

  @override
  State<OnlinePdfOkuyucuSayfasi> createState() =>
      _OnlinePdfOkuyucuSayfasiState();
}

class _OnlinePdfOkuyucuSayfasiState extends State<OnlinePdfOkuyucuSayfasi> {
  final PdfViewerController _pdfViewerController = PdfViewerController();
  final GlobalKey<SfPdfViewerState> _pdfViewerKey = GlobalKey();

  // Varsayılan Test Linki
  final String _dummyUrl =
      "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf";

  late String _aktifPdfUrl;

  // Okuma Ayarları & State
  double _fontSize = 18.0;
  bool _isTextMode = false;
  bool _isDarkMode = false;
  String _extractedText = "Metin indiriliyor ve hazırlanıyor...";
  bool _isTextLoading = true;
  String? _seciliMetin; // Buton kontrolü için
  ...
  void initState() {
    super.initState();
    _aktifPdfUrl = widget.pdfUrl.isNotEmpty ? widget.pdfUrl : _dummyUrl;

    // Sayfa açıldıktan hemen sonra kullanıcıya link sor
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _urlGirisPenceresiAc();
    });
  ...
  // --- URL GİRİŞ DİYALOĞU ---
  Future<void> _urlGirisPenceresiAc() async {
        ...
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
              hintText: "https://ornek.com/kitap.pdf",
              hintStyle: TextStyle(
          ...
          actions: [
            TextButton(
              onPressed: () {
                ...
                if (_aktifPdfUrl.isEmpty) {
                  setState(() => _aktifPdfUrl = _dummyUrl);
                  _metniHazirla();
                }
              },
              child: const Text(
            ...
            ElevatedButton(
              onPressed: () {
                if (urlController.text.trim().isNotEmpty) {
                  setState(() {
                    _aktifPdfUrl = urlController.text.trim();
                    _yukleniyor = true;
                    _isTextLoading = true;
                    _extractedText = "Metin indiriliyor...";
                  });
                  _metniHazirla(); // Yeni link için metni tekrar çek
                }
                Navigator.pop(context);
              },
              child: const Text("PDF'i Getir"),
            ),
          ],
        );
      },
  ...
  // --- METİN AYIKLAMA (ONLINE) ---
  Future<void> _metniHazirla() async {
    try {
      final response = await http.get(Uri.parse(_aktifPdfUrl));
      ...
      if (mounted) {
        setState(() {
          _extractedText = "Metin ayıklanamadı. PDF modunda okuyun.";
          _isTextLoading = false;
        });
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 1,
        title: const Text("Online Okuyucu", style: TextStyle(fontSize: 14)),
        actions: [
          // Yeni Link Butonu
          IconButton(
            icon: const Icon(Icons.link),
            tooltip: "Yeni Link Gir",
            onPressed: _urlGirisPenceresiAc,
          ),
          // Sayfa Numarası
          InkWell(
  ...
  Widget _buildPdfMode() {
    return SfPdfViewer.network(
      ...
      onDocumentLoaded: (details) {
        ...
        int sonSayfa = DbService.sonSayfayiGetir(_aktifPdfUrl);
        if (sonSayfa > 1) {
          Future.delayed(const Duration(milliseconds: 300), () {
            _pdfViewerController.jumpToPage(sonSayfa);
          });
        }
      },
      onPageChanged: (details) {
        setState(() => _mevcutSayfa = details.newPageNumber);
        DbService.sonSayfayiKaydet(_aktifPdfUrl, details.newPageNumber);
      },
      onDocumentLoadFailed: (details) {
        setState(() => _yukleniyor = false);
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("Hata: ${details.description}")));
      },
    );
  }

  // --- SÖZLÜK UI BİLEŞENLERİ ---
